...
function Main() {
	...
	return (
		<>
						...
						<DropdownMenu className='pos-dropdown__dropdown-menu'>
							<DropdownContent>
								<DropdownItem>
									...
									<span className='truncate'>INV-0206020 - {$f()[3].users[0].name}</span>
								</DropdownItem>
								<DropdownItem>
									<Lucide icon='Activity' className='w-4 h-4 mr-2' />
									<span className='truncate'>INV-0206022 - {$f()[4].users[0].name}</span>
								</DropdownItem>
								<DropdownItem>
									<Lucide icon='Activity' className='w-4 h-4 mr-2' />
									<span className='truncate'>INV-0206021 - {$f()[5].users[0].name}</span>
								</DropdownItem>
							</DropdownContent>
						</DropdownMenu>
					</Dropdown>
				...
				<div className='intro-y col-span-12 lg:col-span-8'>
					...
					<div className='grid grid-cols-12 gap-5 mt-5'>
						...
						<div className='col-span-12 sm:col-span-4 2xl:col-span-3 box bg-primary p-5 cursor-pointer zoom-in'>
							<div className='font-medium text-base text-white'>Pancake & Toast</div>
							<div className='text-white text-opacity-80 dark:text-slate-500'>8 Items</div>
						</div>
						<div className='col-span-12 sm:col-span-4 2xl:col-span-3 box p-5 cursor-pointer zoom-in'>
						...
						<TabPanel>
							<div className='box p-2 mt-5'>
								{$_.take($f(), 5).map((faker, fakerKey) => (
									...
									>
										<div className='max-w-[50%] truncate mr-1'>{faker.foods[0].name}</div>
										<div className='text-slate-500'>x 1</div>
										<Lucide icon='Edit' className='w-4 h-4 text-slate-500 ml-2' />
										<div className='ml-auto font-medium'>${faker.totals[0]}</div>
									</a>
								))}
							</div>
							...
							<div className='flex mt-5'>
								...
								<button className='btn btn-primary w-32 shadow-md ml-auto'>Charge</button>
							</div>
						</TabPanel>
						<TabPanel>
							<div className='box p-5 mt-5'>
								...
								<div className='flex items-center border-b border-slate-200 dark:border-darkmode-400 py-5'>
									...
									<Lucide icon='Users' className='w-4 h-4 text-slate-500 ml-auto' />
								</div>
			...
			>
				...
				<ModalBody className='grid grid-cols-12 gap-4 gap-y-3'>
					...
					<div className='col-span-12'>
						...
						<input id='pos-form-3' type='text' className='form-control flex-1' placeholder='People' />
					</div>
				</ModalBody>
			...
			>
				<ModalHeader>
					<h2 className='font-medium text-base mr-auto'>{$f()[0].foods[0].name}</h2>
				</ModalHeader>
				<ModalBody className='grid grid-cols-12 gap-4 gap-y-3'>
					<div className='col-span-12'>
					...
					<div className='col-span-12'>
						...
						<textarea id='pos-form-5' className='form-control' placeholder='Item notes'></textarea>
					</div>
				</ModalBody>
				<ModalFooter className='text-right'>
					<button
